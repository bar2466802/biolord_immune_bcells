...
if __name__ == "__main__":
    ...
    for i, (n_latent_attribute_categorical, reconstruction_penalty, unknown_attribute_penalty,
        ...
        # schedule job only after the previous one has ended
        cmdline0 = ['sbatch', '--gres=gpu:a5000:1', '--mem=100gb', '-c1', '--time=3:00:00',
                    # '--killable',
                    f'--priority={index}',
                    '--parsable',
        ...
        if job_id is not None:
            cmdline0.append(f'--dependency=afterany:{str(job_id)}')
